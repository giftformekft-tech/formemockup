(function($){
  $(function(){
    var $legacyInput = $('#mg-bulk-files');
    if (!$legacyInput.length) {
      return;
    }

    function log(msg){
      $('#mg-bulk-log').append($('<div/>').text(msg));
      var el = $('#mg-bulk-log')[0];
      if (el) {
      ...
      return $.ajax({ url: MG_AJAX.ajax_url, method: 'POST', data: fd, processData: false, contentType: false })
        .done(function(resp){
          if(resp && resp.success){
            log('OK: '+file.name+' → ID '+resp.data.product_id);
          } else {
            log('Hiba: '+file.name+' → '+ (resp && resp.data && resp.data.message ? resp.data.message : 'ismeretlen hiba'));
          }
        }).fail(function(xhr){
