(function($){
    ...
    function initSurchargeBox($box) {
        var data = window.MGSurchargeProduct || null;
        if (!data || !$box.length) {
            return;
        }
        if ($box.data('mgSurchargeInit')) {
            return;
        }
        $box.data('mgSurchargeInit', true);
        ...
        function embedIntoVariantDisplay() {
            ...
            $section.data('mgSurchargeInit', true);
            $box.remove();
            $box = $section;
    ...
    function scanBoxes() {
        $('.mg-surcharge-box[data-context="product"]').each(function(){
            initSurchargeBox($(this));
        });
    }
