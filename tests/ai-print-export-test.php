...
function raw_step($job_id) { return call_hidden('MG_Order_Design_Download', 'process_export_step', $job_id); }
$assertions = 0;
...
try {
    ...
    foreach (array('gpt-image-2.5-sunburst', 'gpt-image-2.5-flare') as $model) {
        ...
        check(step($model)['done'], 'selected model completes quantity export: ' . $model);
    }
    ...
    check(step('legacy-model')['done'], 'pre-upgrade export completes');
    ...
    expect_error(fn() => step('worker-deadline'), '5 percen belül');
    ...
    $http_hook = function () use ($late_before_key) {
        ...
        expect_error(fn() => step('late-before-retry'), '5 percen belül');
    };
    ...
    check(step('late-before-retry')['done'] && count($http_calls) === $before_late_retry_calls, 'valid result arriving after the watchdog but before explicit retry is reused without another API call');
    foreach (array('Allowed memory size exhausted: private-path' => 'memóriakeretét', 'Maximum execution time exceeded: private-path' => 'PHP-futásidőkorlátja') as $fatal_text => $expected) {
        ...
        expect_error(fn() => step('fatal-diagnosis'), $expected);
    ...
    $fallback_progress = step('fallback');
    ...
    check(count($http_calls) === $fallback_calls + 1 && step('fallback')['done'], 'fallback completes and late scheduler delivery does not duplicate the edit');
    ...
    expect_error(fn() => step('recovery'), 'Export folytatása');
    ...
    check(step('recovery')['completed'] === 2, 'successful quantity copies rebuild from the retained image');
    $http_mode = 'ok';
    ...
    check(step('recovery')['done'], 'recovered export completes');
    ...
    check($new_export_key !== $stale_key && step('new-export')['done'], 'a new export completes while the previous export still has a stale running state and lock');
    expect_error(fn() => step('stale-worker'), 'megszakadt vagy nem indult el');
    ...
    check(step('stale-worker')['done'], 'stale worker lock cannot block the explicit replacement attempt');
    ...
    $http_hook = function () use ($late_key) {
        ...
        expect_error(fn() => step('late-worker'), 'megszakadt vagy nem indult el');
    ...
    check(step('late-worker')['done'], 'late response leaves the replacement job usable');
    ...
    step('upscale-failure');
    $upscale_action = end($actions);
    MG_AI_Print_Generator::run($upscale_action[1][0]);
    expect_error(fn() => step('upscale-failure'), '3×-os felnagyítása');
    ...
    do { $result = step($review_job['job_id']); } while (!$result['done']);
    ...
    check(step($mixed_job['job_id'])['waiting'], 'mixed review waits only for the chosen AI item');
    $mixed_action = end($actions);
    MG_AI_Print_Generator::run($mixed_action[1][0]);
    check(step($mixed_job['job_id'])['done'], 'mixed reviewed export completes');
    ...
    step('defringe-disabled');
    $disabled_action = end($actions);
    MG_AI_Print_Generator::run($disabled_action[1][0]);
    check(step('defringe-disabled')['done'] && MG_Image_Utils::$defringed === $defringe_before, 'disabled correction keeps generation and export working');
    MG_Image_Utils::$defringe_failure = true;
    $legacy_task = $tasks[0];
    $legacy_task['ai_defringe'] = true;
    make_job('defringe-failure', array($legacy_task));
    step('defringe-failure');
    $failed_action = end($actions);
    MG_AI_Print_Generator::run($failed_action[1][0]);
    check(step('defringe-failure')['done'], 'legacy enabled task skips correction even when helper would fail');
    ...
    check(step($normal_job['job_id'])['done'], 'ordinary-only reviewed export still works');
