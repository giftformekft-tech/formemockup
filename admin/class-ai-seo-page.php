...
class MG_AI_SEO_Page {
    ...
    public static function render_print_settings() {
        ...
        echo '<tr><th scope="row">' . esc_html__('Kontúrhaló csökkentése', 'mockup-generator') . '</th><td><input type="hidden" name="mg_ai_print_settings[defringe_enabled]" value="0" /><label><input type="checkbox" name="mg_ai_print_settings[defringe_enabled]" value="1"' . checked(MG_AI_Print_Generator::get_defringe_enabled(), true, false) . ' /> ' . esc_html__('Az AI-nyomatok áttetsző peremének igazítása a belső kontúr színéhez', 'mockup-generator') . '</label><p class="description">' . esc_html__('A 3×-os nagyítás és a feketeeltávolítás előtt fut, legfeljebb 3 pixeles sávban, minden kontúrszínnél. A teljesen fedő részeket és az átlátszóságot megőrzi. Szándékos, áttetsző díszszegélynél kapcsold ki. Csak az új exportokra érvényes, az alapmintákra nem.', 'mockup-generator') . '</p></td></tr></table>';
